        ...
        function renderFlagsTable() {
            ...
            flags.sort(function(a, b) {
                switch (_flagSortCol) {
                    ...
                    case 'prod': {
                        // tri par durée en prod ; hors-prod (null) toujours en bas
                        var pa = (a.prodSinceDays != null) ? a.prodSinceDays : (a.inProd ? (a.debtDays || 0) : -1);
                        var pb = (b.prodSinceDays != null) ? b.prodSinceDays : (b.inProd ? (b.debtDays || 0) : -1);
                        return dir * (pa - pb);
                    }
                    default:        return 0;
                }
            ...
            // Colonne Âge : durée depuis la création de la FF.
            function ageCell(f) {
                var ageC = ac(f.ageInDays);
                return '<span class="age-chip" style="background:' + ageC + '22;color:' + ageC + ';">' + f.ageInDays + 'j</span>';
            }

            // Colonne En prod : durée en production — le signal de dette réel.
            //   • pas en prod        → tiret discret (hors prod)
            //   • date estimée       → "~Xj" coloré selon l'ancienneté (debtDays :
            //                          audit events si dispo, sinon temps à 100%)
            ...
            function prodCell(f) {
                ...
                var d = (f.prodSinceDays != null) ? f.prodSinceDays
                      : (f.debtDays != null ? f.debtDays : null);
                if (d != null) {
                    var pc = ac(d);
                    var tip = (f.prodSinceDays != null)
                        ? 'Estimation depuis les audit events GitLab'
                        : 'Estimation depuis la durée à 100% de rollout (audit events indisponibles)';
                    return '<span class="age-chip" style="background:' + pc + '22;color:' + pc + ';" title="' + tip + '">~' + d + 'j</span>';
                }
                return '<span style="font-size:12px;color:#34d399;font-weight:600;" title="En production (durée non déterminable)">✓ en prod</span>';
            }
