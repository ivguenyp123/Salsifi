...
`;
        ...
        async function updatePipelineStatus() {
            try {
                ...
                if (['success', 'failed', 'canceled'].includes(pipeline.status)) {
                    ...
                    if (pipeline.status === 'success') celebratePipelineSuccess();
                }
        ...
        // Feedback visuel quand la pipeline réussit : onde verte + halo sur le monitor
        function celebratePipelineSuccess() {
            // Onde verte qui traverse le fond
            let flash = document.querySelector('.success-flash');
            if (!flash) {
            ...
            void flash.offsetWidth; // reset anim
            flash.classList.add('fire');

            // Halo vert apaisé sur le bloc monitor
            const monitor = document.getElementById('pipelineMonitor');
            if (monitor) {
                monitor.classList.add('run-success');
                setTimeout(() => monitor.classList.remove('run-success'), 3000);
            }
        }
